...
def sample_euler_ancestral(model, x, sigmas, extra_args=None, callback=None, disable=None, eta=1., s_noise=1., noise_sampler=None):
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        x = x + d * dt
        if sigmas[i + 1] > 0:
            x = x + noise_sampler(sigmas[i], sigmas[i + 1]) * s_noise * ((sigmas[i + 1] - sigmas[i + 1] * sigmas[i + 1] / sigmas[i]) / s_max)
    return x
...
@torch.no_grad()
def sample_dpmpp_random(model, x, sigmas, extra_args=None, callback=None, disable=None, s_churn=0., s_tmin=0., s_tmax=float('inf'), s_noise=1.):
    """Implements Algorithm 2 (Heun steps) from Karras et al. (2022)."""
    extra_args = {} if extra_args is None else extra_args
    s_in = x.new_ones([x.shape[0]])
    s_end = sigmas[-1]
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if i == 0:
            # Euler method
            x = sigmas[i + 1] / sigmas[i] * x + (1 - sigmas[i + 1] / sigmas[i]) * denoised
        else:
            # DPM++ with random weight
            w = random.uniform(0, 1)
            x = sigmas[i + 1] / sigmas[i] * x + (1 - sigmas[i + 1] / sigmas[i]) * (w * denoised + (1 -w) * denoised_old)
        denoised_old = denoised

    return x

@torch.no_grad()
def sample_dpmpp_2m_heun(model, x, sigmas, extra_args=None, callback=None, disable=None, s_churn=0., s_tmin=0., s_tmax=float('inf'), s_noise=1.):
    """Implements Algorithm 2 (Heun steps) from Karras et al. (2022)."""
    extra_args = {} if extra_args is None else extra_args
    s_in = x.new_ones([x.shape[0]])
    s_end = sigmas[-1]
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if i == 0:
            # Heun
            x_e = sigmas[i + 1] / sigmas[i] * x + (1 - sigmas[i + 1] / sigmas[i]) * denoised
            denoised_new = model(x_e, sigmas[i+1] * s_in, **extra_args)
            x = x_e + 0.5 * (1 - sigmas[i] / sigmas[i+1]) * (denoised - denoised_new)

        elif i != 0 and sigmas[i+1] != 0:
            # Heun & DPM++ 2M
            x_e = sigmas[i + 1] / sigmas[i] * x + (1 - sigmas[i + 1] / sigmas[i]) * denoised
            denoised_new = model(x_e, sigmas[i + 1] * s_in, **extra_args)
            x = x_e + 0.25 * (1 - sigmas[i] / sigmas[i + 1]) * (denoised - denoised_new) + 0.25 * ((sigmas[i].log() - sigmas[i+1].log()) / (sigmas[i-1].log() - sigmas[i].log())) * (denoised - denoised_old)
        else:
            # euler
            x_e = sigmas[i + 1] / sigmas[i] * x + (1 - sigmas[i + 1] / sigmas[i]) * denoised
            x = x_e
...
name2sampler_func = {'Euler':sample_euler,
                     ...
                     'DPM++ Random':sample_dpmpp_random,
                     'DPM++ 2M Heun':sample_dpmpp_2m_heun,
